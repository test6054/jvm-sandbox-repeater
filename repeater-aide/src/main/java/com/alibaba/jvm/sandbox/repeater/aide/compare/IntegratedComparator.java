...
public class IntegratedComparator implements Comparable {
    ...
    public void dispatch(Object left, Object right, List<Path> paths, List<String> excludeField) {
        ...
        for (Comparator comparator : comparators) {
            if (comparator.accept(left, right)) {
                comparator.compare(left, right, paths, this, excludeField);
                break;
            }
        }
    }
    ...
    @Override
    public CompareResult compare(Object left, Object right, List<String> excludeField) {
        // try clear last compare result
        tryClear();
        // dispatch compare task
        List<Path> paths = new ArrayList<Path>(0);
        try {
            dispatch(left, right, paths, excludeField);
        } catch (Exception e) {
            // LogUtil.error("error occurred when dispatch compare task", e);
