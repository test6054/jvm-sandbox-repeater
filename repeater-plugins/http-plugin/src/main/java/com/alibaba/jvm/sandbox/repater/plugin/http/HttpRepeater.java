...
public class HttpRepeater extends AbstractRepeater {
    ...
    protected Object executeRepeat(RepeatContext context) throws Exception {
        ...
        Map<String, String> headers = rebuildHeaders(context,hi.getHeaders(), extra);
     ...
     * @return 重组后的headers
     */
    private Map<String, String> rebuildHeaders(RepeatContext context,Map<String, String> headers, Map<String, String> extra) {
        if (MapUtils.isEmpty(headers)) {
            return extra;
        }
        //替换headers
        Map<String, String> repleyHeaders = context.getMeta().getExtension();
        headers.putAll(repleyHeaders);
        // 移除掉录制时候的traceId
        headers.remove(Constants.HEADER_TRACE_ID.toLowerCase());
        headers.putAll(extra);
        LogUtil.info("replay request headers {}",headers);
        return headers;
    }
}
