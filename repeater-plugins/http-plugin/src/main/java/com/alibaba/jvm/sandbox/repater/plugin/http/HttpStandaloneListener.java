...
public class HttpStandaloneListener extends DefaultEventListener implements InvokeAdvice {
    ...
    protected void doBefore(BeforeEvent event) throws ProcessControlException {
        ...
        if (!matchRequestURI(patterns, reqUri)) {
            LogUtil.debug("current uri {} can't match any httpEntrancePatterns, ignore this request", reqUri);
            Tracer.getContext().setSampled(false);
            return;
        }
        reqUri = reqUri + ":" + req.getMethod().trim();
        //过滤黑名单
        List<String> blackPatterns = ApplicationModel.instance().getConfig().getBlackEntrancePatterns();
        if(matchRequestURI(blackPatterns,reqUri)){
            Tracer.getContext().setSampled(false);
            return;
        }
        //FIXME 修改采样率
        Map<String, Integer> apiSampleRate = ApplicationModel.instance().getConfig().getApiSampleRateResult();
        LogUtil.info("request apiSAmpleRate : {}",apiSampleRate);
        Integer sampleRate = 0;
        //如果有接口采样率就使用接口采样率，没有就使用统一的采样率
        if(apiSampleRate.get(reqUri) != null){
            sampleRate = apiSampleRate.get(reqUri);
        }else {
            sampleRate = ApplicationModel.instance().getSampleRate();
        }
        if(sampleRate != null && sampleRate > 0 && isSample(sampleRate)) {
            Tracer.getContext().setSampled(true);
            LogUtil.info("采样：" + reqUri);
        }else {
            Tracer.getContext().setSampled(false);
            LogUtil.info("不采样" + reqUri + ",采样率：" + sampleRate);
            return;
        }
    ...
    private boolean isSample(Integer sampleRate) {
        return Tracer.getContext().inApiTimeSample(invokeType,sampleRate);
    }
    ...
    public void onResponse(WrapperRequest request, WrapperTransModel wtm) {
        ...
        try {
            //Http录制方式，解决保存数据乱码问题
            wtm.setResponse(new String(wtm.copier.getResponseData(), "UTF-8"));
        } catch (Exception e) {
