...
public interface RecordRepository extends JpaRepository<Record, Long>, JpaSpecificationExecutor<Record> {
    ...
    @Query(
            "select id,appName,environment,host,entranceDesc,request from Record " +
                    " where appName =  :#{#appName}" +
                    " and traceId in (:#{#traceIdList})"
    )
    List<Record> findByAppNameAndTraceIdList(String appName, List<String> traceIdList);
}
