...
public class TraceContext {
    ...
    public boolean inApiTimeSample(InvokeType invokeType,Integer sampleRate) {
        // 第一级入口流量才会计算采样；非自身入口类型直接抛弃
        if (this.invokeType == null || this.invokeType.equals(invokeType)) {
            boolean sampled = isValid(traceId) && parseLong(getSampleBit(traceId)) % 10000 < sampleRate;
            this.invokeType = invokeType;
            this.sampled = sampled;
            return sampled;
        } else {
    ...
    public static void main(String[] args) {
        System.out.println(ApplicationModel.instance().getSampleRate());
        String traceId = "TR002000001079162633094988310001";
        System.out.println(parseLong(getSampleBit(traceId)) % 10000);
        //TR00200000 1079162633 09498 83100 01
        boolean sampled = isValid(traceId) && parseLong(getSampleBit(traceId)) % 10000 < ApplicationModel.instance().getSampleRate();
